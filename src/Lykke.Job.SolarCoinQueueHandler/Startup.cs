...
namespace Lykke.Job.SolarCoinQueueHandler
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            builder.RegisterModule(new JobModule(appSettings.SolarCoinQueueHandlerJob, log));
            ...
            else
            {
                ...
                {
                    pool.AddDefaultConnection(appSettings.SolarCoinQueueHandlerJob.TriggerQueueConnectionString);
                });
            }
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.SolarCoinQueueHandlerJob.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                logToAzureStorage = new LykkeLogToAzureStorage("Lykke.Job.SolarCoinQueueHandler", new AzureTableStorage<LogEntity>(
                    dbLogConnectionString, "SolarCoinQueueHandlerLog", logToConsole));
